...
class FeatureEngineer:
    ...
    def euclidean_distances(self, field1, field2):
        return np.sqrt(np.sum((self.df[field1] - self.df[field2]) ** 2, axis=1))

    def difference_vectors(self, field1, field2):
        return self.df[field1] - self.df[field2]

    def inner_product(self, field1, field2):
        return np.dot(self.df[field1], self.df[field2])

    def vector_product(self, field1, field2):
        return np.cross(self.df[field1], self.df[field2])
...
class Embedder:
    def __init__(self, data_file, output_file, chunk_size=500, batch_size=16):
        ...
        self.model = SentenceTransformer("google/embeddinggemma-300m", device=self.device)
    ...
    def embed_fields(self):
        ...
        with open(self.output_file, 'w') as out_file:
            for i in tqdm(range(0, len(data), self.chunk_size), desc="Processing chunks"):
                ...
                # Batch encode fields with progress bar for embeddings
                with tqdm(total=4, desc="Embedding fields", leave=False) as pbar:
